...
- **Two ADRs recorded** — `docs/decisions/0002-consensus-critical-aggregation.md` (consensus ≥2 votes over the falsified max-over-reviewers rule) and `docs/decisions/0003-workflow-scriptpath-contract.md` (the empirically-proven committed-Workflow scriptPath contract: body-execution + top-level `return await main(args)`, pure-literal `meta`, `check-bundle.mjs` wrap-parse over `node --check`). ADR 0003 flags that the `planning-brain` seed predates this proof and **must be audited against the contract before M2 relies on it** (it may still carry the phantom `export { main }` / `version`/`entry` shape).
