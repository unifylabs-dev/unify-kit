...
- **Scope note — this is Tier-1.** It pins the hook *scripts'* fire+block contract over the exact tool-call envelopes loop-spawned agents emit, and converts the M0 prose claim into an executed, red-capable test. The **Tier-2** proof — the 7 hooks firing on a *real* loop-spawned `agent()`-issued Edit/Bash inside a running Workflow (the substrate-coverage half, which GitHub Actions cannot run: no `claude` CLI/credentials on the runner) — remains a **human-witnessed local run**, proposed to bundle with the M1 P6 live eval. The M0 "no coverage gap over the new substrate" claim is therefore still `asserted-not-proven` until Tier-2 lands.
