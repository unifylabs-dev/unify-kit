...
### Added (phasing-flow initiative — M4 new capabilities: a `/workflow-library` reference recipe + a detect-only `doc-freshness` routine pilot)
- **`/workflow-library` ships as a curated docs catalog + a committed ADR-0003 recipe seed — NOT a slash command** (ADR 0008, D1). The first recipe, `review-changed-files` (`plugins/unifylabs-workflow/workflows/recipes/review-changed-files/`), is a **read-only** fan-out: `parallel()` one reviewer per dimension (correctness / security / error_handling) over a `git diff`, reach **Critical-precision consensus** (a Critical survives only if ≥`critVotes` distinct reviewers tier it within ±`tolerance` lines — the ADR-0002 rule re-expressed **recipe-locally** in a tested `lib/consensus.mjs`, NOT a copied lib, so the recipe is copy-pasteable and the CI harness drops the `cmp` byte-identity guard), then **adversarially verify** each confirmed Critical (fail-OPEN). The catalog (`docs/workflow-library.md`) indexes the recipe and cross-links the three internal seeds (planning-brain, the engine, the integrate-branch audit) as worked examples. **Proven live** — run `wf_3c45b7ca-f8c`: 8 agents over a staged planted-bug target → 11 findings, 5 Criticals confirmed (e.g. RCE via `eval`, an unguarded `JSON.parse`, a null-deref, a hardcoded `sk-live-` secret), each verified `refuted:false`, zero working-tree mutation. The kernel's own tests caught two real bugs during the build (`Number(null)===0` let an unlocatable line vote/dedup as line 0 — fixed via a `toFiniteLine` guard).
- **A detect-only `doc-freshness` ROUTINE pilot** — the kit's first **routine** (the scheduled, **detect-and-report-only**, never-mutate maintenance tier). `scripts/doc-freshness-scan.sh` is a repo-intrinsic, `$HOME`-free, **provably read-only** detector for 5 deterministic signals (count drift, ADR-index gaps, empty `[Unreleased]`, skills without a doc entry, living-docs lagging recent churn) — unlike `check-drift.sh`, which is entirely machine-local. The routine recipe (`plugins/unifylabs-workflow/workflows/routines/doc-freshness/`) runs the script + adds **LLM judgment** for the signals a script cannot do precisely (citation drift, architecture-lag) and reports via a single **idempotent `gh issue` find-or-update** — the one allowed GitHub-state write; never a repo-tree mutation. **Proven live** (the detect substance): baseline `✓ FRESH` (exit 0, zero false positives) + a true-positive (a stub 14th skill, no count ripple → 3 correct signals, exit 1) then reverted clean. The FP-heavy dangling-citation signal (32 false positives on the clean tree — shorthand / aspirational / historical paths) was deliberately moved OUT of the deterministic script and into the routine's judgment layer — exactly the agentic value a cloud routine adds over a cron Action (D5).
- **Front-loaded PROBE → branch P-c (ADR 0008).** The canonical routine primitive is `RemoteTrigger` (`/v1/code/triggers`; the on-disk `schedule` skill's `create_scheduled_task` is its wrapper; `CronCreate` is session-local and unfit). But read-only `RemoteTrigger list` returned **HTTP 401** twice — claude.ai is unauthenticated in this build env — so the cloud routine **registration is documented as a consumer step** (no cloud run id was fabricated), per the G1-approved fallback. Never-mutate is stated honestly: it rests on the read-only script + prompt posture + origin-log-inspectable single-issue emit; an API tool-allowlist was not verifiable here. The critic also **removed a proposed "security-hook backstop" never-mutate layer** (verified: no kit hook blocks `git commit`/`push`, and a cloud routine loads none of this repo's hooks).
- **Seed/docs/script shape ⇒ ZERO count ripple, ZERO version bump** (plugin stays v2.1.0; 13 skills / 17 commands / 9 hooks unchanged — a recipe/routine/script is not a skill/command/hook). The only CI change is **one new `review-recipe-harness` job (node --check + node --test + bundle parity + a RED self-test that sabotages the consensus rule) → 20 → 21 jobs.** Additive + revert-clean: delete the recipe/routine dirs + the script + the catalog + this ADR + restore the 20-job narrative, and `RemoteTrigger delete` any registered cloud routine. **ADR 0008** records all five decisions (D1–D5), the P-c probe, and the design overrides.

### Changed (phasing-flow initiative — M5 migrate + coexist: a soft, per-use-case `phasing` ↔ `phasing-flow` coexistence pointer — the LAST milestone)
- **`phasing` and `phasing-flow` now coexist via a SOFT, per-use-case routing pointer — NOT a wholesale deprecation; `phasing` survives intact and may be improved for the use cases it still fits (ADR 0009).** The `phasing` SKILL gains a blockquote callout (no new heading) stating the routing rule: prefer `phasing-flow` for verifiable multi-file **code** builds where plan quality is the risk (few high-signal gates, serial single-session); keep `phasing` for judgment/non-code work, multi-terminal/parallel isolation, a human gate at every transition, crash/checkpoint recovery, or GitHub-issue-per-phase tracking. The routing is grounded in a 10-use-case map — only UC-1 (a multi-subsystem code build with a real deterministic verifier) clearly prefers `phasing-flow`; the rest stay on `phasing` (UC-10, non-code/prose work, because the engine resolves no verifier on a prose spec and silent-passes — zero deterministic verification; the ADR-0007 finding). **Nothing is deleted, renamed, or version-bumped** — the plugin stays v2.1.0; 13 skills / 17 commands / 9 hooks unchanged; CI stays 21 jobs (M5 is prose + governance, no new testable seed).
- **The narrative surfaces are retargeted with additive routing clauses (every existing `phasing` recommendation preserved).** The impact-matrix Theme-6 docs (`docs/curated-plugins.md` — `phasing-flow` ADDED alongside the kept `phasing` citation; the onboarding curriculum / intro / day-30 / week-1) plus the 6 surfaces a completeness critic caught that the matrix missed (the consumer `cheatsheet` template — an in-row clause, no new row, preserving its `8 daily commands` self-count; the `llms.txt` template; `docs/methodology.md` §E and the mirror `specs/07` §D; `docs/philosophy.md`; `specs/02` prose — never its `(8)` count enumeration). The 5 `docs/audit/*` files get a flag-only "historical snapshot (pre-phasing-flow-initiative)" banner (body byte-stable — framed as a point-in-time snapshot, so it does not over-claim staleness).
- **Governance:** ADR 0009 (`accepted` — a coexistence-model decision, not a removal) + its index row; a `BACKLOG.md` "M5 follow-up (deferred)" tracker for the per-use-case deprecate-or-keep decision (deferred to after migration; nothing moved or closed); a light design-time historical note on `specs/2026-05-23-handoff-skill-design.md` §9 (its checkpoint surface survives). **No `UPGRADING.md` section** (M5 is non-breaking). The `[local]` `~/.claude/CLAUDE.md` "Phasing check" gate is retargeted off the stale `phased-execution` name onto `phasing-flow` (out-of-tree; not part of this PR).
- **Built the dogfood way (M5 closes the initiative).** A 5-reader **Understand** workflow (`wf_ecff6f1d-e27`) mapped the 10 use-cases and verified the Theme-6 ripple surface against the live tree — its completeness critic caught 6 missed narrative surfaces, 2 ghosts, and 1 already-satisfied item; the live **planning-brain** seed (`wf_9e261b09-62c`, `ground:false`) synthesized the master plan whose adversarial critic caught a double count-coupling (the `specs/02` `(8)` enumeration) and forced one canonical ADR slug; the three diff-moving choices went to a human GATE 2. **ADR 0009** records the decision. **M0→M5 are all landed — the phasing-flow initiative is complete.**

### Changed (phasing-flow initiative — M3 adoption, skill 3 of 3: spec-it evaluated — planning-brain wire DECLINED by a pre-registered blind probe)
- **`spec-it` stays on its existing 11-phase in-session flow — the planning-brain wire was DECLINED on empirical evidence, completing M3 (adoption).** spec-it is planning-shaped (deliverable = a prose spec + a `gh issue create` filing; no per-AC code diff, no resolvable verifier), so the execution engine is the wrong tool (it would `resolveVerifier → []` and silent-pass) — the only candidate seam was the live `planning-brain` seed. Per the **gate-on-decisions** discipline, OD-1 (where/whether to wire) was NOT pre-decided: a pre-registered, **blind, 3-grader probe** drove the seed AS-IS on its *strongest* framing (Phase-4 decomposition — the only native `steps[]` fit) against spec-it's current single-pass flow on a real spec idea (magic-link auth). The seed scored **2 of 5** decomposition dimensions, below the **≥3-of-5** bar written *before* the run → **NO-OP**. The decisive, probe-independent finding: the single-pass baseline beat the seed on the **structural** dimensions — **partition + dependencies** — that are Phase-4 decomposition's actual job (exactly as the `steps[]`-vs-unordered-spec **shape-mismatch** predicts), while the seed's wins (completeness + AC-rigor) were either recovered downstream (AC-rigor lands in the draft the **Phase-7 `iterative-review`** already reviews) or a residual completeness gap accepted *because* the seed was simultaneously worse at its core job. All 3 graders preferred the seed holistically; the pre-registered per-dimension bar (locked before any run) is honored over the holistic preference.
- **Floor fixes only (the impact-matrix:67 "update phasing refs" clause) — three sites across two files:** the stale `/work-issue` **Phase-3.5** reference at `spec-it/SKILL.md:233` (Phase 3.5 was deleted by ADR 0006) is corrected to "absorbs the whole thing as one oversized issue"; the softer phasing ref at `:523` is reconciled; and a third stale Phase-3.5 ref in `references/decomposition-heuristics.md:4` is corrected. **No pointer block, no `Workflow` in `allowed-tools`, no frozen-region guard, no new CI job — CI stays 20 jobs; the `/spec-it` consumer contract is byte-for-behavior unchanged. No skill/command/hook count ripple.**
- **Built the dogfood way.** A 5-reader **Understand** workflow (`wf_fa3a1ca1-eee`) produced a file:line-anchored facts brief; the live **planning-brain** (`wf_be35d256-652`) synthesized the master plan whose **adversarial critic** elevated NO-OP from "first-class but unlikely" to the **leading hypothesis to falsify** (the seed's lenses are wrong for both drafting and decomposition; `steps[]` is execution-plan-shaped while spec-it's artifacts are unordered; Phase-7 already reviews the draft; "advisory" risks a round-trip for output the human re-decides at Gate 4 anyway). The front-loaded **probe** then resolved OD-1 empirically: ARM-SEED `wf_75ec458e-5b9` (non-null → null-rate 0) vs the in-session baseline, blind-graded `wf_f8ad756d-9d8` against the pre-registered `PROBE-RUBRIC.md`. **ADR 0007** records the decision; the impact-matrix:67 "adopt engine" divergence is ratified as "adopt planning-brain" (then declined on evidence). A dogfood loop that can honestly decline a wire is native-first-but-quality-first working as designed.

### Changed (phasing-flow initiative — M3 adoption, skill 2 of 3: work-issue re-platformed onto the framework)
- **`work-issue` is re-platformed onto the phasing-flow framework INTERNALLY — the `/work-issue N` 8-phase issue→PR consumer contract is preserved byte-for-behavior.** Unlike the integrate-branch audit seed (skill 1 of 3), work-issue ships **NO new compute kernel / bundle / parity-harness** — it has no parity-lockable arithmetic, so its wiring is a **SKILL REWIRE that DRIVES the already-live `planning-brain` + `phasing-flow-engine` seeds by scriptPath** (two additive `### Run via the committed seed (M3)` pointer blocks). **Phase 3** drives the **planning-brain** seed for **M/L** issues (with a mandatory null-guard on `result.master_plan` → in-session prose fallback, never advancing without an approved plan); **Phase 4** hosts the per-AC TDD body on the **execution engine** as the **verify + adversarial diff-review GATE** (**Mode B**, S/M/L conditional, one AC = one unit) — RED→GREEN→REFACTOR authoring stays **in-session** (`superpowers:test-driven-development`) because the engine is a generic agent, not a TDD enforcer, and a deterministic verify cannot prove RED-first. The 4 verify-spine gaps the engine does NOT cover (`npx tsc --noEmit`, scope guard, modified-test check, the non-empty-resolveVerifier assertion) are documented as POST-engine orchestrator duties.
- **Phase 3.5 (the phasing detector) is DELETED + its `--phase` / `--no-phase` flags removed** — the engine becomes the Phase-4 host, killing the over-offer surface; the deletion scrubbed all 4 verified ripple sites (the block, the flag bullets, the count line, the Phase-4 "if phasing was invoked" note). **Phase 5.5 (e2e/visual fidelity) is KEPT as a HUMAN-GATED orchestrator check** — never routed through the engine's fail-OPEN diff-review layer (a running Workflow takes no human input mid-run, so the authoritative visual gate must live in the orchestrator session). The `phasing` SKILL itself is untouched — only work-issue's internal Phase-3.5 call-site is deleted. **11 phase-stops → 3 decision gates + 2 SDD addenda** (Phase-0 spec-sync incl. the Step-4a CLAUDE.md non-negotiables cross-check; Phase-7 spec-diff guard).
- **Additive + revert-clean, proven by a 10-block content-anchored frozen-region guard.** `skills/work-issue/test/frozen-regions.test.mjs` + `test/golden/{01..10}.md` assert the genuinely-untouched SKILL blocks (gate convention, master-protection, Pre-flight, Phase 0/1/2/5/5.5/6/7) stay BYTE-VERBATIM, the two seed pointers landed, and Phase 3.5 + the flags are gone — **13 zero-dep `node:test` cases**, red-capable (a drifted golden turns it red). A new **`work-issue-frozen-harness`** CI job runs it + a RED gate-the-gate; **CI is now 20 jobs** (the honest M3.1 mirror — the frozen test needs its own host because `structural-validation` has no `node` step and every `node --test` job is seed-scoped; "CI stays 19/19" was a phantom the planning-brain critic caught). **No skill/command/hook count ripple** (a `test/` dir is not a skill dir).
- **ADR-0004's deferred `fixed-point` obligation is discharged:** the dead `fixedPointK` pass-through is removed from the engine `wrapper.mjs` (the kernel never destructured it) and the committed engine bundle regenerated (35 `node:test` cases + bundle parity stay green); the stall guard is recorded as CONDITIONAL on a future parallel/retry cursor, keyed on a collision-free `id` (NOT `(id,title)` — the duplicate-label landmine), RED-testable when it binds.
- **Built the dogfood way.** A 6-reader **Understand** workflow (`wf_50353a0c-83d`) produced a file:line-anchored facts brief; the live **planning-brain** (`wf_dacd4c66-a44`) synthesized the P0..P8 master plan whose **adversarial critic** corrected all three scout proposals (the 19/19→20/20 phantom-host; Mode A-vs-B is a design decision not probe-decidable; the probe needs a no-test-script fixture) and surfaced the open decisions resolved at the approve-plan gate. A **front-loaded live probe** confirmed Mode-B wirability BEFORE the rewire: a deterministic `resolveVerifier` check (with-test → non-empty; no-script → `[]`) + two live engine runs — `wf_45463258-703` (`clean`, 1/1) and `wf_edb0c262-86b` (RED verify → `criticals-pending-gate`, invariant intact). **ADR 0006** records the twelve decisions. Engine-backed salvage for `integrate-branch` is now transitively real but **CONDITIONAL (M/L only)**.

### Added (phasing-flow initiative — M3 adoption, skill 1 of 3: the integrate-branch audit seed)
- **`integrate-branch`'s Phase-2 audit is re-platformed onto the FOURTH committed Workflow seed** (`skills/integrate-branch/workflow/`, ADR-0003 scriptPath contract, after `iterative-review` + `planning-brain` + the `phasing-flow` engine). The 6-dimension audit is now a typed `parallel()` (D4 nests `parallel(6 pr-review-toolkit reviewers)` → consensus; D2 = scout → role-tagged `resolveVerifier`) reduced by a pure, import-free `compute-audit.mjs` kernel (every SKILL.md scoring formula verbatim + the composite + the ÷0.95 D6 renormalization + FAIL-CLOSED blocking overrides with Discard>Rebuild precedence + the route matrix). The seed returns `{scores, composite, route, override, degraded, signals}` with **NO mid-run gate** — the salvage/rebuild/discard route gate + the detached-worktree lifecycle stay in the SKILL session (a running Workflow takes no human input). **Additive + revert-clean:** the D1–D6 prose is retained as the spec + fallback; revert = delete `workflow/` + one SKILL pointer block. **Runs LIVE** — end-to-end smoke `wf_026d5240-bce` (the committed bundle via scriptPath; all 6 dimensions ran, ÷0.95 renorm, composite 88 → `salvage`).
- **The agent→reduce bridge (`audit-aggregate.mjs`) + the verdict-parity acceptance harness.** D4 consensus runs at **critVotes=1 (dedup-only)** — the 6 semantically-distinct `pr-review-toolkit` lenses rarely co-locate, so the engine's `critVotes=2` (tuned for shared-lens reviewers) would have gutted D4's critical signal. D2 adopts `resolveVerifier` + a **synthetic `npx tsc --noEmit` fallback** when a `tsconfig.json` exists but no `typecheck` script — closing the silent type-check-gate hole de-hardwiring would otherwise open. The literal "byte-identical to the current audit" acceptance is unachievable (the current audit is prose; consensus + resolveVerifier change behavior by design), so acceptance is **reduce-parity, two-gate**: Gate A (an inert fixture ⇒ byte-identical composite + route to an INDEPENDENT `legacyReduce`) + Gate B (documented consensus/D2 deltas vs pre-registered goldens).
- **New `integrate-audit-harness` CI job** (`plugin-install-fixture.yml`): `node --check`, **44 zero-dep `node:test` cases** (`compute-audit` 23 + `audit-aggregate` 12 + `verdict-parity` 6 + `frozen-regions` 3), the **3-copied-lib byte-identity guard** (`canonical`/`consensus-aggregate`/`verifier-detect` vs the `iterative-review` originals — NOT the engine's 7-lib loop, the concrete CI landmine), bundle parity + `check-bundle` + import-free, and a **RED self-test** (mutate a `compute-audit` weight ⇒ the suite must turn red). A **content-anchored frozen-region guard** asserts the SKILL's Phase-3 route gate + Phase-4 handoffs stay byte-verbatim (immune to line drift).
- **Built the dogfood way:** `/phasing-flow plan` ran the live **planning-brain** (run `wf_0b27698d-8a8`) whose adversarial critic caught five factual errors in the scout proposals (consensus returns an array not counts; +6-not-+10 per demotion; the named agentTypes unproven; `resolveVerifier`'s silent tsc-drop; the 7-lib `cmp` landmine) and surfaced five open decisions resolved at the approve-plan gate; a **front-loaded agent-resolution probe** (`wf_8bf0957a-0cc`) proved the named `pr-review-toolkit` agentTypes resolve + emit line-bearing findings BEFORE the full build. **ADR 0005** records the decisions. **No skill/command/hook count ripple** (a workflow seed, like the engine/planning-brain). Engine adoption for salvage/rebuild is **transitive via `/work-issue`** and a NO-OP for this change (it becomes real only after M3 #2 lands).

### Added (phasing-flow initiative — M2 proper: the execution engine + skill + single `/phasing-flow` command)
- **The flagship `phasing-flow` execution engine** — a new `phasing-flow` SKILL at `plugins/unifylabs-workflow/skills/phasing-flow/` whose `workflow/` dir is the **third committed Workflow seed** under the ADR-0003 scriptPath contract (after `iterative-review` + `planning-brain`). It generalizes the enforced loop-until-dry to a serial loop over an approved `units[]` plan: per unit `executeUnit → deterministic verify (AUTHORITATIVE) → adversarial consensus-≥2 diff-review (fail-open)`, exiting on one of the frozen 7 reasons. Pure kernel (`execution-engine.mjs` + `verify-verdict.mjs` + `wrapper.mjs`) over injected callbacks; `src/glue.mjs` the only runtime-touching file; deterministic `build-workflow.mjs` + `check-bundle.mjs`; 35 zero-dep `node:test` cases (red-capable). **Runs LIVE** — smoke `wf_ddbd2cbb-581` (clean, 1/1, a real execute→verify→diff-review cycle) + `wf_2cc8aa35-512` (a RED `make test` → `criticals-pending-gate`).
- **Hardened by an adversarial kernel review** (`wf_0cf2db43`, 5 lenses, pre-bundle) that caught two real bugs — a zero-baseline circuit-breaker false-abort (`5×0=0` tripped on any spend; the baseline is now captured after the first unit's clean cycle) and a `diffReview`-throw that broke the fail-open contract (a throwing reviewer crashed the run instead of being coerced to 0) — and proved a Set-based `fixed-point` ceiling is premature for M2's strictly-advancing serial loop AND a false-positive landmine on duplicate-labeled units. **fixed-point is deferred to M3**; the loop is bounded by the unit count + the `cap` ceiling. Verification fails CLOSED on the authoritative deterministic verify (a throw / permanent failure / unrecognized verdict → `aborted`) and OPEN on the adversarial diff-reviewer. (ADR 0004.)
- **`phasing-flow` skill core** — `SKILL.md` (the orchestrator gate-holding spine) + 5 references (`engine-contract`, `gates`, `verification-spine`, `planning-brain-wiring`, `status-card`). It drives the human-gated loop brainstorm → planning-brain Workflow → **approve-plan gate** → execution-engine Workflow → **verification sign-off gate** → next unit. A running Workflow takes no mid-run input, so all three human gates live in the orchestrator session, BETWEEN runs.
- **Single `/phasing-flow` command** with verb subcommands (`start` / `plan` / `run` / `verify` / `status` / `resume`) — one command, not a `phase-*`-style family (avoids re-introducing the count-drift class M0 fixed). `/phasing-flow plan` wires onto the live `planning-brain` seed IN PLACE (relocate deferred to M3).
- **New `phasing-flow-engine-harness` CI job** (`plugin-install-fixture.yml`): `node --test` (red-capable), bundle parity (regenerate + `git diff`), `check-bundle`, import-free, the **copied-lib byte-identity diff-guard** vs the `iterative-review` originals (the 7 generic `lib/` modules are copied verbatim — single source of truth, NOT a cross-seed import), and a RED self-test (a disabled ceiling MUST fail the suite).
- **Counts reconciled** to **13 skills / 17 commands / 9 hooks** across the fixture assertions, `plugin.json` (**v2.0.3 → v2.1.0**) + `marketplace.json` descriptions, `CLAUDE.md`, both READMEs, `dev-symlink-skills.sh`, and the onboarding docs. The stale "8 hooks" current-actual claim in `specs/2026-05-30-orchestration-framework-design.md` + the impact-matrix is corrected to 9; the `CLAUDE.md §2` "M2 relocates the planning-brain seed" note is corrected to "M3 relocates" (M2 wires it in place). **ADR 0004** records the engine decisions.

### Fixed (phasing-flow M2 exit proof — the `ci-pr-fast` snippet dogfood)
- **`templates/snippets/ci/ci-pr-fast.yml.template` no longer hard-fails an adopted `ci.yml` on `main` when the project defines no `test` script.** The Tier-1 "Unit tests (CI split)" step now guards on the Node case: when a `package.json` exists but declares no `test` script, the step emits a GitHub Actions `::warning::` and skips (`exit 0`) instead of running `{{TEST_CI_CMD}}` into a "Missing script: test" failure; stacks without a `package.json` (Python, Go, Ruby, …) run the command unchanged. Closes the v1.0.1 BACKLOG follow-up surfaced by `unify-kit-example-nextjs`'s failing CI on `main`.
- **This fix was the M2 exit proof** — it shipped *by* the `phasing-flow` engine dogfooding itself end-to-end (run `wf_d1124974-485`): one human-gated run through lightweight-plan → **approve-plan gate** → engine `run` (`executeUnit → deterministic verify → adversarial consensus-≥2 diff-review`) → **verification sign-off gate**, exiting `clean` (1/1 units, 0 residual). The kit's own authoritative checks at sign-off were all GREEN — the in-engine deterministic verify resolves no verifier for the kit (pass-by-default), so the real gate is the kit's `actionlint` **v1.7.12 (= CI's exact version)** on the token-substituted workflow + `scrub-check` (forbidden-strings + placeholder-vocab) + the diff-review's no-consensus-Critical. **M2 (the execution engine + skill + single `/phasing-flow` command) is complete.**

### Changed (phasing-flow initiative — M2: planning-brain adapter — the seed now runs LIVE)
- **The `planning-brain` Workflow seed was adapted to the proven scriptPath contract (ADR 0003) and now RUNS LIVE** (smoke run `wf_3e271589-060`: `ground:false` → 3 angled planners + adversarial critic + judge, returning the frozen `{master_plan, critique, angled_plans, scout_count}` with the judge writing its master plan to `plansDir` and surfacing `open_decisions_for_human` as `{decision, options, recommendation}` triples). The committed P5 seed carried the same phantom shape that blocked `iterative-review` (audited 2026-06-04, ADR 0003) — its 26 tests covered the kernel + wiring + bundler determinism, NOT a live run. Three phantom-API fixes in `workflows/planning-brain/src/glue.mjs`: (1) `meta` `{name, version, entry, inputs}` with a `+`-concatenated description → **pure-literal `{name, description, phases}`** (the `+` is a Workflow-tool-rejected BinaryExpression; declared-inputs doc folded into the description); (2) `main(inputs={})` → `main(rawArgs)` with a defensive `parseArgs` (the `args` global arrives as a **JSON string**); (3) the phantom `return phase('planning-brain', async () => {…})` wrapper removed — the real `phase(title)` returns void, so it would have made the workflow result `undefined`; stage grouping already comes from the `phase:` field on each `agent()` opts. The `agent(promptString, {label, phase, schema})` calls were already the real signature (generic agents — no `agentType`), so they were unchanged.
- **`build-workflow.mjs`** now appends a top-level **`return await main(typeof args !== "undefined" ? args : {})`** entrypoint instead of the dead `export { main };` (the runtime executes the script body — there is no exported-entry auto-invocation — and the top-level return is the workflow result).
- **New `workflows/planning-brain/check-bundle.mjs`** (ported from the `iterative-review` reference) replaces `node --check` for the bundle: it wrap-parses the body as an async function (the runtime shape — a bare-module `node --check` raises "Illegal return statement" on the correct bundle) and asserts zero imports, sole `export const meta`, **meta is a pure literal**, and a top-level `main()` entrypoint. CI (`planning-brain-harness`) excludes the bundle from the find-based `node --check` and validates it via `check-bundle.mjs` in the parity step — mirroring the `loop-harness` swap. Bundle parity + import-free + tracked-file guards unchanged; the 26 `node:test` cases still pass.
- **Docs:** `workflows/planning-brain/README.md` reconciled (inputs arrive via the JSON `args` parsed by `main()`, not declared in `meta`; the regenerate recipe uses `check-bundle.mjs` not `node --check`; new `check-bundle.mjs` row + the live-run reference). M2 can now wire `/phasing-flow plan` onto a seed that actually executes.

### Fixed (handoff resume — `freshness-check.sh` false-`fatal`) + CI coverage for the handoff scripts
- **`freshness-check.sh` no longer false-`fatal`s a resume from a different clone / worktree / sibling repo.** It probed the caller's **cwd** instead of the handoff's frontmatter `project_root`, so a resume from anywhere other than the original project reported every repo-relative load-bearing path as a "missing" file (→ `overall: fatal`, which refuses auto-resume) and read git HEAD/branch as "drift". It now resolves the git state, the load-bearing files, and the `run.json` path against `project_root` (falling back to cwd for older handoffs that predate the field), and tests load-bearing entries **by kind** — directory entries (trailing slash) via `-d`, glob entries (`*`/`?`/`[`) via expansion, files via `-e` — instead of a blanket `-f` that also reported directories and unexpanded globs as missing. (Hit live resuming the M1-gate handoff from a sibling clone — diagnosed as a cwd/path-resolution false alarm, exactly as that handoff's resume note predicted.)
- **New `handoff-script-tests` CI job** (`plugin-install-fixture.yml`) runs the handoff skill's three resume-script harnesses (`test-freshness-check` / `test-detect-mode` / `test-recreate-tasklist`) on every push — they shipped in v2.0.3 with fixtures but **zero CI coverage**, which is why the regression shipped. `test-freshness-check.sh` gains a **cross-cwd regression case** (relative load-bearing paths + a directory entry + a glob, run from a non-project cwd) that is `fatal` on the old behavior and `clean` on the fix.

### Added (phasing-flow initiative — M1 engine↔runtime adapter: the iterative-review Workflow runs LIVE)
- **`iterative-review` now executes on the real Workflow runtime** (previously asserted-not-proven — the P3 bundle was built against a phantom API and could not run). `workflow/src/glue.mjs` was rewritten to the **proven** contract (probe `wf_5ffc08f3-c30` + the N=3×6-reviewer de-risk + a live smoke run `wf_96c7adc7-bde` that exited `criticals-pending-gate` and scored **PASS** on the no-regression gate, recall 2/3 / FP 0): `agent(promptString, { agentType, schema })` (not the object form); the 6 `pr-review-toolkit:*` reviewer **agentTypes** (verified to resolve + behave as fixture reviewers); `meta = { name, description, phases }` as a **pure literal** (no string concatenation); the runtime executes the script **body** (no exported-entry auto-invocation) so the bundler now appends a top-level `return await main(args)` entrypoint; and `args` arrives as a **JSON string** so `main()` parses it defensively. The three placeholder agent types that did not exist (`fixer` / `verifier-scout` / `verifier-runner`) are replaced by the default workflow agent (fixer) and a scout→`resolveVerifier`→runner verify path over real agents. Review callbacks now close over the resolved `mode`/`target` (the engine calls `reviewRound` without them — a latent bug in the first cut). The report returns the consensus-aggregated initial findings so the no-regression live gate can score detection quality. Reviewers cite the **diff-target path** (the hunk's `+++ b/<file>` header), not the patch/container file — a worst-of-N gate run caught that container-path citations could reach consensus and register as critical false positives; pinning the citation convention removes the intermittent FP.
- **Critical-precision consensus stage — `workflow/lib/consensus-aggregate.mjs`** (+ 10 zero-dep `node:test` cases, incl. a gate-the-gate regression mirroring the captured de-risk). The M1 de-risk proved a naive 6-reviewer union FAILS the trust gate's FP≤0 criterion: one reviewer stochastically scored a gold-Important missing-`await` at 90 → Critical → worst-of-N FP=1. The stage replaces max-over-reviewers aggregation with **consensus**: a Critical survives only if ≥2 distinct reviewers independently tier a Critical within ±3 lines (else demote to Important), then dedup by the canonical key. **Validated before the build** — applying it to the captured live transcripts and re-scoring with the unmodified `run-no-regression-eval.mjs` flips the verdict FAIL→PASS (worst recall 2/3, worst FP 0). `references/severity-policy.md §Aggregation` updated to encode this (the old "pick the highest severity" rule is the one the de-risk falsified).
- **`workflow/check-bundle.mjs`** — a zero-dep validator that replaces `node --check` for the generated bundle (the bundle's top-level `return` is intentionally not legal in a bare ESM module — the Workflow tool wraps the body in an async function). It wrap-parses the body as an async function (the runtime shape) and asserts the runtime contract CI otherwise cannot verify: zero imports, `export const meta` is the SOLE export, **meta is a pure literal** (statically rejects concatenation / template / spread / call — exactly what the runtime rejects), and a top-level `main()` entrypoint is present (the phantom bundle exported `main` but never ran it).
- **CI (`plugin-install-fixture.yml` `loop-harness`)**: the generated bundle is excluded from the find-based `node --check` and validated by `node check-bundle.mjs` in the parity step instead; the byte-for-byte bundle-parity, import-free, and tracked-file guards are unchanged. The new consensus test runs under the existing `node --test` step (62 workflow tests pass).
- **P6 live gate — objective half PASS** (evidence committed at `skills/iterative-review/evals/p6-live-evidence/`). The committed bundle was driven **N=3** via `Workflow(scriptPath)` over the isolated fixture; worst-of-3 Critical-recall **2/3 (= baseline)**, FP **0 (= baseline)**, no false-clean → the scorer's objective verdict is PASS and re-runnable (`node run-no-regression-eval.mjs p6-live-evidence/runs`).
- **P6 live gate — security Tier-2 witness + sign-off → the binding M1→M2 trust gate is DISCHARGED (2026-06-04)** (evidence at `skills/iterative-review/evals/p6-live-evidence/security-tier2/`). The human half that no credential-less CI runner can produce was captured live: two loop-spawned agents inside a running Workflow (runs `wf_27829dbb-5de`, `wf_9f708a0f-a26`) — the engine's **verbatim fixer dispatch** (`agent(fixPrompt(group,wd),{label:'fix'})`) attempting a guarded `.env` write was **hard-blocked by `file-guard`** (PreToolUse; the file never reached disk, a control write in the same dir succeeded), and a loop agent's `Read` of a fake-AWS-example-key fixture **tripped `output-secrets-scanner`** (PostToolUse, exit 2). The active session hooks were confirmed byte-identical to the shipped hooks. With **Tomer sign-off** on both the objective N=3 faithfulness and this security witness, the gate (design §12, §13, §14 #3, §119) is discharged and **M2 is unblocked**. `dangerous-actions-blocker` (Bash) is not live-fired by design (no safe non-destructive command matches it); its coverage rests on Tier-1 + the now-twice-proven hooks-fire-on-Workflow-subagents mechanism.

### Changed (phasing-flow initiative — M1 P7 close-out)
- **Two ADRs recorded** — `docs/decisions/0002-consensus-critical-aggregation.md` (consensus ≥2 votes over the falsified max-over-reviewers rule) and `docs/decisions/0003-workflow-scriptpath-contract.md` (the empirically-proven committed-Workflow scriptPath contract: body-execution + top-level `return await main(args)`, pure-literal `meta`, `check-bundle.mjs` wrap-parse over `node --check`). ADR 0003 records that the `planning-brain` seed was **audited 2026-06-04 and confirmed to carry the phantom shape** (`meta={name,version,entry,inputs}` + uncalled `export { main }`) — it will not run live as-is and needs the same engine↔runtime adapter treatment before M2 relies on it.
- **Doc reconciliations:** the stale `phased-execution`→`phasing` skill name corrected across `work-issue/SKILL.md` (the shipped skill is `phasing`); the `CLAUDE.md §7` PR-merge shellcheck command now includes `plugins/unifylabs-workflow/hooks/test/*.sh` (matching the CI `lint` glob); and the M1 **P3** CHANGELOG entry carries a reconcile note (its bundle's "runnable" claim was asserted-not-proven until the engine↔runtime adapter).

### Added (phasing-flow initiative — M1 P6 "no-regression trust gate — Tier-1 machinery")
- **The binding M1→M2 trust-gate scorer** — `skills/iterative-review/evals/run-no-regression-eval.mjs`, a standalone zero-dep Node script that scores `iterative-review` run transcripts against the frozen code-mode benchmark and decides **worst-of-N Critical-recall vs the human-gated baseline**. PASS (objective half) = worst-of-N Critical-recall ≥ baseline worst (2/3) **AND** every run surfaces ≥1 Critical (no false-clean / skip-if-clean) **AND** worst-of-N critical false-positives ≤ baseline (0, precision not regressed). It **bakes in the load-bearing line-coordinate normalization** the P0 baseline did by hand: reviewers cite diff-text `+`-line numbers, gold lines are post-image source lines, so the scorer normalizes `source = cited − hunkStartLine(file)` (the fixture-text line of each file's `@@` header — 24/81/128) **before** the ±3 match. A naive raw compare floors recall at 0 — the hazard `gold-findings.json.line_coordinate_convention` warns about. Matching follows the gold `matching_hint`: same file + nearest gold within ±3 source lines + severity-tier → **full** match (the gold defect is "caught", once); tier mismatch → **partial** (caught-but-miscalibrated, does **not** count toward Critical-recall — the score-90 borderline calibration probe); an unmatched Critical-severity finding → a critical false-positive.
- **16 zero-dep `node:test` cases** for the scorer, incl. the **self-test that reproduces `baseline.json`** from the three committed transcripts (2/3, 2/3, 3/3 → worst 2/3, FP 0), the calibration probe (borderline scored Important → partial, not caught), the ±3 boundary, false-clean detection, false-positive/precision, JSON + markdown-table parsing (always reading the *cited* column, never a pre-computed source-line column), worst-of-N aggregation, and a **RED gate-the-gate** test proving that breaking the normalization (zero offsets) floors Critical-recall at 0.
- **`no-regression-harness` CI job** in `plugin-install-fixture.yml`, mirroring `loop-harness`: `node --check` + `node --test`, the **Tier-1 self-test** (scorer reproduces `baseline.json`), a **positive control** (the baseline transcripts pass the live-runs CLI path), and a **RED self-test** (a deliberately false-clean run MUST be rejected non-zero). The scorer is a standalone script (not a bundled Workflow), so there is no parity step. Pinned Node 20, zero-dep.
- **`no-regression-protocol.md`** — the two-tier gate contract: Tier-1 (offline, CI; proves the *scorer* sound, does **not** discharge the gate) vs Tier-2 (live, human-witnessed; the real engine reviews the benchmark **N≥3** times + the security live witness + **Tomer sign-off**). Documents the live-run citation convention and the turnkey Workflow invocation for the gated session. **Tier-1 GREEN here does not certify M1** — the binding gate still requires the N≥3 live witnessed runs + the formal security Tier-2 (`file-guard` blocks a loop-spawned fixer + `output-secrets-scanner` fires, captured as a committed artifact) + Tomer sign-off, none of which can run on a credential-less CI runner.

### Added (phasing-flow initiative — M2 seed: the planning-brain Workflow)
- **`planning-brain` Workflow seed** under a new `plugins/unifylabs-workflow/workflows/` staging dir — the twice-dogfooded planning-brain pattern committed as a reusable, self-contained, **importer-free** Workflow seed, mirroring the `iterative-review` precedent exactly (tested `.mjs` source → deterministic bundler → committed self-contained bundle → CI parity + no-import + tracked-file guard). The pattern: **Ground** (parallel scout agents read state into structured briefs) → **Plan** (parallel angled planners over an `ANGLES` table, each a different lens) → **Critique** (one adversarial critic reads ALL surviving plans) → **Judge** (synthesizes ONE master plan, incorporates the critic's fixes, surfaces `open_decisions_for_human` as `{decision, options, recommendation}` triples for the human gate). Split into a PURE `lib/planning-core.mjs` (the `ANGLES` table + planner-spec construction + the `.filter(Boolean)` salvage + lean critic/judge input assembly + the frozen result builder), an injection-based `wrapper.mjs` (`runPlanningBrain` over injected `runScouts`/`runPlanners`/`runCritic`/`runJudge` callbacks — unit-testable with stubs), an `agent()`/`parallel()`-backed `src/glue.mjs` (the only runtime-touching file; holds `meta` + the LEAN per-stage schemas), a deterministic `build-workflow.mjs`, and the generated `planning-brain.workflow.mjs` bundle.
- **Lean-schema discipline baked in** (this session a planning brain failed because planners used a ~14-field nested schema with a hyphenated key, never called `StructuredOutput`, and returned `null`): planner schemas are LEAN + flat-ish; snake_case keys ONLY (never hyphenated); heavy plan content is written to disk via a manifest agent; and a **mandatory downstream salvage** drops `null`-returning planners (`.filter(Boolean)`) so the critic + judge still run, with the judge explicitly told some planners may have returned nothing. The salvage + planted-gap-wiring + determinism are guarded by 26 zero-dep `node:test` cases. CI cannot test planning *quality* (live-model / human eval); these tests guard **wiring + salvage + determinism** only.
- **`planning-brain-harness` CI job** in `plugin-install-fixture.yml`, mirroring the `loop-harness` job exactly: `node --check` every `.mjs`, `node --test` the suite (fail if zero tests), bundle parity (`node build-workflow.mjs` then `git diff --exit-code`), the no-import grep guard, and the tracked-file assertion. Pinned Node 20, zero-dep. **Not** a skill/command/hook — no count ripple; M2 later relocates the seed under the `phasing-flow` skill and wires `/phasing-flow plan`.

### Added (phasing-flow initiative — M1 verification spine: `verifier-backstop` Stop hook)
- **9th plugin hook — `verifier-backstop.sh` (Stop event).** An additive, **default-OFF** opt-in workflow hook that deterministically re-runs the project's real verifier *fresh* and **blocks the stop on a live RED** — the no-LLM enforcement half of the M1 verification spine (a model asked to honor a prose "verify before sign-off" rule is not the same as a deterministic re-run that gates the stop). The plugin hook count reconciles to **9 hooks = 7 security/integrity + 2 workflow** (the 2 workflow hooks are `context-awareness` + `verifier-backstop`); the **7 security/integrity sub-count is unchanged**. Count prose reconciled across `plugin.json`, `marketplace.json` (byte-identical mirror), the plugin README (prose + a 9th hook-table row), `CLAUDE.md` §2, root `README.md`, `llms.txt` (both sites), `docs/onboarding/intro.md`, `onboarding/day-1.md`, `specs/03-hooks.md` (9th-hook note + summary), and `specs/2026-05-30-orchestration-framework-impact-matrix.md` Theme 9. Security-only rosters/headers and version-stamped historical claims left intact.
...
- Resolved open decisions #2–#5 and renamed the `flow` shorthand to **`phasing-flow`** across `specs/2026-05-30-orchestration-framework-design.md` + its impact matrix (the Workflow-tool "workflow"/"Workflow" term preserved). **M0 asserted** (M1 P4 converts this to executed proof — see the P4 Tier-1 entry below) that the 7 security/integrity hooks fire *and enforce* on Workflow-spawned in-process agents — no coverage gap over the new substrate. Added a current-hook-count note to `specs/03-hooks.md`, and added spec 14 + the dated design docs to the `specs/README.md` index.
...
### Added (phasing-flow initiative — M1 P0 "gate-the-gate + baseline")
- **First executed-JS CI gate** — a `loop-harness` job in `plugin-install-fixture.yml`: zero-dependency `node --test` / `node --check` over `skills/iterative-review/workflow/` on pinned Node 20, triggered by the existing `plugins/**` filter. The kit otherwise runs no Node tooling; this is the substrate the enforced loop-until-dry needs, since a loop's ceilings are only *provable* by running them (a prose rule a model is asked to honor is not). Proven live: a deliberately-failing assertion turns the gate red, the placeholder smoke test green. (Node-stack-expansion ADR lands in M1 P7.)
- **Frozen code-mode review benchmark + N≥3 baseline** under `skills/iterative-review/evals/benchmark/` — a deterministic unified-diff fixture with 6 known-severity planted defects (3 Critical incl. a score-90 borderline calibration probe and a separate-file Critical for the skip-if-clean check, 2 Important, 1 Suggestion), a hidden `gold-findings.json` manifest, clean + unresolvable artifacts, and an `npm test` verifier. Captured today's human-gated reviewer's **worst-of-3 Critical-recall floor (2/3)** in `baseline.json` + committed transcripts — the score the M1 enforced loop must meet-or-beat (the M1→M2 no-regression trust gate).

### Added (phasing-flow initiative — M1 P1+P2 "enforced stopping engine")
- **The enforced loop-until-dry stopping engine** at `skills/iterative-review/workflow/` — a pure, side-effect-free, `agent()`-free JS kernel (`stopping-engine.mjs` + `lib/`) that re-homes the five prose stopping rules into **code-enforced ceilings**: max-rounds `clamp(cap,1,5)` (clamp-with-warning, not silent); fixed-point detection on a **full-tuple canonical key** (`file,line,severity,normalized-description` — upgraded from the legacy `desc[:80]` that collided distinct findings) with strict-shrink + set-swap + **cycle/oscillation** detection; and a **dual budget breaker** (relative-5× *and* native-absolute, fires on either — runaway protection survives even with no token budget set). Adapted to the Workflow platform fact that a running workflow takes no mid-run input: Criticals are **collected, never fixed in-loop**, and a new `criticals-pending-gate` exit reason hands them to the between-runs human gate, taking precedence so a Critical-bearing run is never mislabeled `fixed-point`/`cap`. Frozen 7-member exit-reason vocabulary with a throwing guard; frozen return shape; every truncation logged (no silent stop by construction). The engine carries NO agent/i/o/wall-clock/random — it is unit-testable with deterministic stubs.
- **22 unit tests generated FROM `workflow/evals/loop-control-evals.json`** (single source of truth — harness and eval cannot silently drift) covering every ceiling + edge case (oscillation, set-swap, relative-budget-with-null-total, skip-if-clean, empty-set→clean, `criticals-pending-gate` at both cap and fixed-point, gate-important, include-suggestions, full-tuple-key collision, residual-promoted-suggestion), plus a recursive determinism source-scan guard (no wall-clock/random) and a frozen-vocabulary guard. Adversarially verified by mutation testing — disabling any ceiling fails a dedicated test. Runs in the `loop-harness` CI job; replaces the P0 placeholder smoke test.

### Added (phasing-flow initiative — M1 P3 "engine wired into the review Workflow")
> **Reconciled by the engine↔runtime adapter (top of `[Unreleased]`):** the bundle committed at P3 was built against a *phantom* Workflow API — "runnable" below was **asserted, not proven**; it could not actually execute on the real runtime. The engine↔runtime adapter proved the real contract and rewrote the glue so it runs live (worst-of-3 objective gate PASS). Read the P3 entry as "wrapper + bundler + parity machinery landed," not "the engine ran."
- **The iterative-review Workflow wrapper + deterministic bundler.** Verified that a Workflow-runtime script **cannot `import` modules** (sandboxed VM, no module loader — dynamic and static both fail), so the tested engine is **inlined into a runnable, self-contained Workflow script** (`iterative-review.workflow.mjs`) by a deterministic bundler (`build-workflow.mjs`). The `.mjs` engine/lib remain the single unit-tested source of truth; a new CI parity step regenerates the bundle and **fails on any drift** (and asserts the bundle is a tracked file, so the guard can't be silently disarmed). The wrapper (`wrapper.mjs`) is **injection-based** — initial-review fan-out, re-review, fixers, and verifier are all injected — so the whole orchestration is unit-testable with stubs; the only `agent()`-using code is the thin generated glue. Fan-out uses **native `parallel()`** (the hand-rolled `min(16,cores-2)` limiter is gone — native + determinism-safe). The loop is worktree-agnostic via `workingDir`-as-arg (clean EnterWorktree handoff). New pure, tested helpers: `mode-detect`, `verifier-detect`, `parse-findings`. The canonical finding key is now `JSON.stringify([file,line,severity,normalized-desc])` — collision-proof and text-safe (replaced NUL/SOH separators that made the bundle git-binary). **52 tests pass** (engine 22 + wrapper 12 + helpers 18) in the `loop-harness` CI job.

### Changed (phasing-flow initiative — M1 P3 docs)
- **iterative-review docs re-pointed at the enforced engine.** `SKILL.md` §5 rewritten to describe the code-enforced loop — the ceilings + the `criticals-pending-gate` relocation of the per-Critical gate to *between* workflow runs (a running Workflow takes no mid-run input), `/goal` reserved for the M2 outer seam — and §Red-flags / §End-state / §6 reconciled to the 7 frozen exit reasons in `exit-reasons.mjs` (no more authoritative cap numbers in prose). `references/stopping-rules.md` collapsed to a pointer (the thresholds now live only in `stopping-engine.mjs`). Worktree refs (`SKILL.md` §3/§6.5, `references/{modes,worktree-handling}.md`) gain a "superseded by native EnterWorktree — migration tracked in M0/Theme-4, not yet landed" banner, plumbing left operational. New `scrub-check` CI guard fails if the prose re-asserts authoritative stopping-rule pseudocode/caps.

### Added (phasing-flow initiative — M1 P4 "security-hook enforcement harness — Tier-1")
- **The kit's first executed-hook test** — `plugins/unifylabs-workflow/hooks/test/test-security-hooks.sh`, a zero-dependency `bash` harness that drives all **7 verbatim security/integrity hooks** with the real hook-event JSON envelope shape a Workflow-spawned agent's tool call produces, and asserts each hook's real enforcement signal: `exit 2` + stderr reason to block, `exit 0` to allow (the advisory `marketplace-drift-check` asserted fire-not-block). 16 assertions, **block + allow paired per hook** — the allow case is a real fail-*closed* guard, since the python3/git-parse-dependent hooks `exit 2` when their tool is absent, which would otherwise mask a false green. **Red-capable two ways** (gate-the-gate, per P0): the normal run must pass (a hook that stops blocking turns it red) and a `HARNESS_SELFTEST_RED=1` inversion must fail (an assertion that stops binding turns it red); `CLAUDE_HOOKS_DISABLE=<hook>` independently drives each enforcing hook red. The 7 hook scripts are **byte-for-byte unmodified** (kept verbatim per design §8/§15) — only test + fixture files are added. Planted secret/bidi literals are split/encoded so the kit's own `forbidden-strings` + bidi scrub-check stay clean. Runs in a new **`security-hook-harness`** CI job in `plugin-install-fixture.yml` (normal run + RED self-test), and the harness is now shellcheck-covered (the `lint` glob widened to `hooks/test/*.sh`).
- **Scope note — Tier-1 + live probe.** Tier-1 pins the hook *scripts'* fire+block contract over the exact tool-call envelopes loop-spawned agents emit (converting the M0 prose claim into an executed, red-capable test). A **live probe** then confirmed the substrate half for the canonical path: a Workflow-spawned `agent()`'s `.env` Write was **hard-blocked by `file-guard`** — un-fakeable (the file never reached disk; a control `.md` write in the same dir succeeded). The remaining **formal, human-witnessed Tier-2** (live `file-guard` + `output-secrets-scanner`, committed as an artifact + sign-off, which GitHub Actions cannot run: no `claude` on the runner) is **bundled with the M1 P6 live eval**. The Bash path (`dangerous-actions-blocker`) cannot be safely live-fired (no non-destructive command matches it); its substrate coverage rests on Tier-1 + the now-proven "PreToolUse hooks fire on Workflow subagent tool calls" mechanism.
