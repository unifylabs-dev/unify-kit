// build-workflow.mjs — the DETERMINISTIC bundler for the iterative-review
// Workflow.
//
...
// resolver. So the tested kernel (lib/*.mjs + stopping-engine.mjs + wrapper.mjs)
// and the agent-backed glue (src/glue.mjs) must be INLINED into ONE
// self-contained script. This bundler does that inlining, and a CI parity step
// (`node build-workflow.mjs` then `git diff --exit-code <bundle>`) proves the
// committed bundle never drifts from a fresh regeneration.
//
...
//
// Run:  node build-workflow.mjs   (writes iterative-review.workflow.mjs)
...
const BUNDLE_PATH = join(__dirname, 'iterative-review.workflow.mjs');
...
//   fixed-point         (needs canonical: isSubset, serializeSet)
