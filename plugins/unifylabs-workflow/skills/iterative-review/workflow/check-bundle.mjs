#!/usr/bin/env node
// check-bundle.mjs — validate the generated iterative-review.workflow.mjs the way
// the Workflow tool actually runs it.
//
// WHY NOT `node --check`: the committed bundle ends with a top-level `return await
// main(args)` (proven by probe wf_5ffc08f3-c30: the runtime executes the script
// BODY — there is no exported-entry auto-invocation — and the top-level return is
// the workflow result). A top-level `return` is NOT legal in a bare ESM module, so
...
// (Bundle determinism — no wall-clock / RNG tokens — is enforced separately by the
// engine's recursive guard in test/stopping-engine.test.mjs, which scans every
// .mjs under the workflow dir INCLUDING this generated bundle. Not re-checked here
// to avoid two scanners drifting.)
