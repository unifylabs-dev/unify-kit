#!/usr/bin/env node
// check-bundle.mjs — validate the generated planning-brain.workflow.mjs the way
// the Workflow tool actually runs it.
//
// WHY NOT `node --check`: the committed bundle ends with a top-level `return await
// main(args)` (proven by probe wf_5ffc08f3-c30, ADR 0003: the runtime executes the
// script BODY — there is no exported-entry auto-invocation — and the top-level
// return is the workflow result). A top-level `return` is NOT legal in a bare ESM
// module, so `node --check <bundle>` raises "Illegal return statement" on the
// CORRECT bundle. The Workflow tool extracts `export const meta` and wraps the rest
// in an async function, where top-level await + return ARE legal. This validator
// mirrors that: it wrap-parses the body as an async function body and asserts the
// structural invariants the runtime + the no-imports rule require.
//
...
//     residual `+` / backtick / `...` / `(` is a non-literal construct. (A `+`
//     INSIDE a description string is fine — it is stripped first.)
...
//     CALL the orchestration (the phantom bundle exported main but never ran it).
...
// (Bundle determinism — no wall-clock / RNG tokens — is a SOURCE-level discipline
// here: glue/wrapper/planning-core carry no Date/Math.random/new Date, and the
// bundle is a deterministic concatenation of those sources guarded byte-for-byte by
// the CI parity step (regenerate + git diff). Not re-scanned here to avoid two
// checkers drifting; the planning-brain seed has no stopping-engine guard.)
