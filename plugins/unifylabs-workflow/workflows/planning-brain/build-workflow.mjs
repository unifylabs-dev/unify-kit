// build-workflow.mjs — the DETERMINISTIC bundler for the planning-brain Workflow.
//
// THE CONSTRAINT (VERIFIED): a Workflow-runtime script CANNOT import modules —
// no dynamic and no static import; it runs in a sandboxed VM with no module
// resolver. So the tested kernel (lib/planning-core.mjs + wrapper.mjs) and the
// agent-backed glue (src/glue.mjs) must be INLINED into ONE self-contained
// script. This bundler does that inlining, and a CI parity step (`node
// build-workflow.mjs` then `git diff --exit-code <bundle>`) proves the committed
// bundle never drifts from a fresh regeneration.
//
...
//     re-running produces a byte-identical bundle.
...
//
// Run:  node build-workflow.mjs   (writes planning-brain.workflow.mjs)
...
const BUNDLE_PATH = join(__dirname, 'planning-brain.workflow.mjs');
...
// respects it.)
//
//   lib/planning-core   (no deps — the ANGLES table + pure assemblers/salvage)
//   wrapper             (needs planning-core: plannerSpecsFromAngles, salvage, ...)
//   src/glue            (needs wrapper + planning-core; holds meta + agent dispatch)
const SOURCES = [
  'lib/planning-core.mjs',
  'wrapper.mjs',
  'src/glue.mjs',
];
 ...
 *   `export default ...`   -> dropped keyword (none in our sources; guard anyway)
...
function stripExports(lines) {
  ...
  for (const line of lines) {
    ...
    // `from` clause. Drop the whole line. Also handles the multi-line
    // `export {\n a,\n b,\n};` form via a small inner state if needed — but our
    // sources keep the closing `};` on the SAME conceptual statement, so a
    // single-line regex plus the multi-line guard below covers them.
    if (/^\s*export\s*\{[^}]*\}\s*;\s*$/.test(line)) {
      continue;
    }
    // Multi-line bare re-export opener `export {` with no closing brace on the
    // line and no `from` — start dropping until the matching `};`.
    if (/^\s*export\s*\{[^}]*$/.test(line) && !/from/.test(line)) {
      out.push({ __dropMultiExport: true });
      continue;
    }
  ...
  // Second pass: resolve the multi-line bare-export drop markers. When we see a
  // __dropMultiExport marker, swallow lines until the closing `};`.
  const resolved = [];
  let dropping = false;
  for (const item of out) {
    if (typeof item === 'object' && item && item.__dropMultiExport) {
      dropping = true;
      continue;
    }
    if (dropping) {
      if (/\}\s*;?\s*$/.test(item)) dropping = false;
      continue;
    }
    resolved.push(item);
  }
 ...
 * the meta block removed. The meta literal is brace-balanced; we scan from the
 * `export const meta` line until the matching close brace.
