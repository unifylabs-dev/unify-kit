...
if ! command -v python3 >/dev/null 2>&1; then
  printf '[hook: %s] python3 not found in PATH; this hook requires python3 to parse the Bash payload. Install python3 or set CLAUDE_HOOKS_DISABLE=%s to acknowledge the bypass; blocking conservatively.\n' "$_NAME" "$_NAME" >&2
...
' 2>/tmp/dangerous-actions-blocker.parse.err)" || _cmd_rc=$?

if [[ "${_cmd_rc:-0}" -ne 0 ]] || [[ "$_cmd" == "PARSE_ERROR" ]]; then
  printf '[hook: %s] payload parse failed: %s; blocking conservatively.\n' "$_NAME" "$(cat /tmp/dangerous-actions-blocker.parse.err 2>/dev/null || echo unknown)" >&2
  rm -f /tmp/dangerous-actions-blocker.parse.err
