...
| **M1** | Reference impl + planning brain | `iterative-review` as enforced `loop-until-dry` Workflow; planning-brain prototype | **proves gates-survive-on-Workflow + verification ≥ human-gated baseline** (no-regression eval — the M1→M2 trust gate, §14 #3) — **✅ trust gate DISCHARGED 2026-06-04** (objective N=3 PASS + security Tier-2 witness + Tomer sign-off; `evals/p6-live-evidence/`). Engine runs live; planning-brain seed is phantom (M2 adapts it). |
...
M1 is the trust gate: we do not tackle the flagship engine until the reference implementation empirically shows native sub-agents don't regress to the performative-DEFERRED-verification failure that killed the old subagent-based skill. **✅ Discharged 2026-06-04** — the enforced `iterative-review` loop met the no-regression bar (objective N=3: worst-of-3 Critical-recall 2/3 = baseline, FP 0 = baseline) and the security substrate was witnessed live (`file-guard` hard-blocks a loop-spawned fixer's guarded write; `output-secrets-scanner` fires on a secret in a loop agent's output), with Tomer sign-off. M2 is unblocked.
...
- **Security-coverage gap** over Workflow-spawned agents. *Mitigation:* **retired (2026-06-04)** — M1 P4 Tier-1 (executed, CI-gated, red-capable harness over all 7 verbatim hooks) + a live probe confirming `file-guard` blocks a Workflow-spawned agent's credential write; the formal human-witnessed Tier-2 (`file-guard` + `output-secrets-scanner` live over a running loop) **landed 2026-06-04** (`evals/p6-live-evidence/security-tier2/`: both hooks fire+block on real loop-spawned Workflow agents, + sign-off). The Bash path (`dangerous-actions-blocker`) cannot be safely live-fired (no non-destructive command matches it); its substrate coverage rests on Tier-1 + the now-proven "PreToolUse hooks fire on Workflow subagent tool calls" mechanism.
...
3. **Trust bar** — ✅ RESOLVED (2026-06-01) — **no-regression eval vs. baseline.** Before building the M2 engine, M1's verification quality on a fixed benchmark task must be ≥ the current human-gated baseline. (Maps to the M1 exit proof in §12.) **✅ DISCHARGED 2026-06-04** — objective N=3 PASS (worst-of-3 Critical-recall 2/3 = baseline, FP 0 = baseline) + the security Tier-2 witness + Tomer sign-off; evidence at `plugins/unifylabs-workflow/skills/iterative-review/evals/p6-live-evidence/` (`runs/` objective half, `security-tier2/` security half).
